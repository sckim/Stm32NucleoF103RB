...
- 이 세 함수 뒤에서 일어나는 일(GPIOA 클럭 활성화, `CRL` 설정, `BSRR` 쓰기, SysTick 기반 `delay`)을 프레임워크가 대신 처리한다는 점. 이후 `01`~`03`에서 이 숨겨진 부분을 계층별로 벗겨 본다.
...
`01_BlinkHAL`(HAL), `02_BlinkLL`(LL), `03_BlinkReg`(레지스터)와 비교한다.
