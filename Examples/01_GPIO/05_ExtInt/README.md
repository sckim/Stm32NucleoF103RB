...
- 이 예제에는 디바운스 처리가 없다. 기계식 버튼의 접점 튐 때문에 한 번 눌러도 여러 번 토글될 수 있는지 관찰해 볼 만하다 (→ `08_Clock_System/78_NonBlocking_StateMachine_HAL_c`에 디바운스 예).

## 관련 예제

폴링 방식 `04_ReadPin`, 여러 EXTI 라인 `07_ExternalInt`, 저전력 웨이크업에 EXTI를 쓰는 `09_WatchDog_Sleep/83_Stop_Mode_EXTI_HAL_c`.
