...
`05_ReadPin`의 레지스터 버전. `GPIOC->IDR`를 직접 읽어 B1(PC13) 상태에 따라 LD2(PA5)를 제어한다. (폴더에 Proteus 시뮬레이션 파일 `STM32F103R.pdsprj`도 포함되어 있다.)
...
버튼이 안 눌린 상태(High)면 LD2 ON, 눌리면 OFF (`05_ReadPin`과 같은 논리). `SysTick_Init()`이 호출되지만 이 예제의 루프는 딜레이를 쓰지 않는다.

## 관련 예제

`05_ReadPin`(HAL 버전), `04_BlinkReg`(출력 레지스터 설정).
