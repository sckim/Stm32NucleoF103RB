...
- **바이트당 인터럽트 → 패킷당 인터럽트**: `02_USART_Interrupt_RX_HAL_c`은 바이트마다 CPU가 호출됐지만, DMA는 CPU 개입 없이 메모리에 쓰고 IDLE 이벤트 때만 CPU를 부른다.
...
`02_USART_Interrupt_RX_HAL_c`(바이트별 인터럽트), `04_USART_RingBuffer_HAL_c`(링 버퍼), DMA 기초 `07_DMA/01_DMA_MemToMem_HAL_c`.
