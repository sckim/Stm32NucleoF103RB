...
WWDG 카운터 틱은 `4096 × 8 / PCLK1(32MHz) ≈ 1.024ms`이다. `Counter = 64(0x40)`에서 시작하면 리셋 임계(0x3F)까지 약 1틱(≈1ms)뿐인데 루프의 리프레시 주기는 50ms이므로, **이 설정에서는 리프레시 전에 리셋이 반복될 가능성이 크다.** 정상 동작(LD2 점멸)하지 않는다면 `Counter`를 `0x7F` 쪽으로 늘리고 `Window`를 그보다 작게 잡는 식으로 타이밍을 다시 계산해 보자. 계산 방법과 동작하는 예는 `81_WWDG_HAL_c` 참고.
...
`81_WWDG_HAL_c`(윈도우 타이밍 계산과 조기 경고), `08_Clock_System/79_DeviceInfo_UniqueID_Reg_c`(리셋 원인 확인).
