/**
 * 06_IAP_Xmodem_Bootloader_Reg_c  —  [계층: 레지스터(CMSIS)]  UART 로 새 펌웨어를 받아 Flash 에 기록하는 부트로더 (XMODEM-CRC)
 *
 * 03_IAP_Bootloader 는 "이미 Flash 에 있는 앱으로 점프"만 했다. 이 예제는 **PC 에서 UART 로 .bin 을 보내 앱 영역에 직접 기록**한다.
 * ST-LINK 없이 현장에서 펌웨어를 업데이트하는 IAP(In-Application Programming)의 전형적인 구조이다. 앱은 `04_IAP_App_Reg_c` 를 사용한다.
 *
 ...
 *   2) 앱 프로젝트(04_IAP_App_Reg_c)를 빌드해 `.pio/build/nucleo_f103rb/firmware.bin` 을 만든다.
...
/* ---------------- 앱 검사/점프 (03_IAP_Bootloader 와 동일) ---------------- */
