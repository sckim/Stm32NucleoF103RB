/**
 * 11_IAP_Bootloader_Reg_c  —  [계층: 레지스터(CMSIS)]  IAP(In-Application Programming) 부트로더: 응용 프로그램으로 점프
 *
 * 짝 예제: 12_IAP_App_Reg_c (응용 프로그램). 두 프로젝트를 각각 빌드해서 플래시의 서로 다른 영역에 올린다.
 *
 * Flash 배치 (F103RB 128KB)
 *   0x08000000 ~ 0x08007FFF (32KB) : 이 부트로더   (ldscript.ld 의 FLASH LENGTH = 32K)
 *   0x08008000 ~ 0x0801FFFF (96KB) : 응용 프로그램 (12_IAP_App_Reg_c, FLASH ORIGIN = 0x08008000)
 *
 * 부트로더가 하는 일
 ...
 *   2) 12_IAP_App_Reg_c 를 빌드/업로드. 업로드 도구는 ELF 의 주소(0x08008000~)에만 기록하므로 부트로더는 지워지지 않는다
